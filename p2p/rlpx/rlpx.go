...
func (c *Conn) Handshake(prv *ecdsa.PrivateKey) (*ecdsa.PublicKey, error) {
	...
	// @notes 辨别该连接是inbound/outbound
	if c.dialDest != nil {
		// @notes active handshake
		sec, err = initiatorEncHandshake(c.conn, prv, c.dialDest)
	} else {
		// @notes passive handshake
...
func initiatorEncHandshake(conn io.ReadWriter, prv *ecdsa.PrivateKey, remote *ecdsa.PublicKey) (s Secrets, err error) {
	...
	// @notes Generate Auth identity information, including signature, public key
	// generated by random nonce corresponding to signature.
	// The generation of random number calls crypto/rand package and uses the
	// kernel function rltgenrandom under windows
	authMsg, err := h.makeAuthMsg(prv)
	if err != nil {
		return s, err
	}
	// @notes Carry out rlpx coding
	authPacket, err := sealEIP8(authMsg, h)
	if err != nil {
		return s, err
	}

	// @notes Initiate encryption handshake and send the generated auth message
	if _, err = conn.Write(authPacket); err != nil {
		return s, err
	}

	// @notes Receive the returned authResp message, verify the decryption,
	// and obtain the public key of the other party.
	// For the specific rlPx encryption and decryption process,
	// see the https://github.com/ethereum/devp2p/blob/master/rlpx.md
